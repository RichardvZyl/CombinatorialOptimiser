...
internal sealed class LinKernighanSolver : ISolver<DistanceMatrix, PermutationResult>
{
    ...
    public PermutationResult Solve(DistanceMatrix m) =>
        SolverRunner.Timed(Name, Paradigm, m, () =>
        {
            ...
            while (improved) { improved = false; for (var i = 0; i < n - 1; i++) for (var k = i + 1; k < n; k++) { var a = permutation[i == 0 ? n - 1 : i - 1]; var b = permutation[i]; var c = permutation[k]; var d = permutation[(k + 1) % n]; if (a == c || b == d) continue; if ((m[a, c] + m[b, d]) - (m[a, b] + m[c, d]) < -1e-10) { PermutationUtils.Reverse(permutation, i, k); improved = true; } } }
            return permutation;
        });
}
