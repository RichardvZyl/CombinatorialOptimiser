...
public sealed class SimulatedAnnealingSolver : ISolver
{
    ...
    public SolverResult Solve(DistanceMatrix m) =>
        SolverRunner.Timed(Name, Paradigm, m, () =>
        {
            ...
            var rng = new Random(RandomSeed); var currentCost = m.TourLength(permutation);
            ...
            while (temperature > MinTemperature)
            {
                for (var step = 0; step < StepsPerTemperature; step++)
                {
                    ...
                    if (delta < 0) { Reverse(permutation, i, k); currentCost += delta; if (currentCost < bestCost) { bestCost = currentCost; Array.Copy(permutation, bestPerm, n); } }
                    else if (temperature > 0 && rng.NextDouble() < Math.Exp(-delta / temperature)) { Reverse(permutation, i, k); currentCost += delta; }
                }
    ...
    private static double Calibrate(DistanceMatrix m, int[] permutation, Random rng)
    {
