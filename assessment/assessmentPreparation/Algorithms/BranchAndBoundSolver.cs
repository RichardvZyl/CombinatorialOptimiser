...
public sealed class BranchAndBoundSolver : ISolver
{
    ...
    {
        var n = m.Count;
        if (n >= 31) throw new InvalidOperationException("Branch and Bound requires n < 31 (got " + n + ").");
        if (n <= 1) return Enumerable.Range(0, n).ToArray();
        if (n == 2) return new[] { 0, 1 };
        var min1 = new double[n]; var min2 = new double[n];
        for (var i = 0; i < n; i++)
        {
            var best = double.PositiveInfinity; var second = double.PositiveInfinity;
            for (var j = 0; j < n; j++) { if (i == j) continue; var d = m[i, j]; if (d < best) { second = best; best = d; } else if (d < second) { second = d; } }
            min1[i] = best; min2[i] = second;
        }
        ...
        void Search(int depth, int last, double currentCost, int mask)
        {
            ...
            for (var next = 1; next < n; next++) if ((mask & (1 << next)) == 0) remaining.Add(next);
        ...
        double LowerBound(int current, int visitedMask)
        {
            ...
            for (var i = 1; i < n; i++) if ((visitedMask & (1 << i)) == 0) sum += min1[i] + min2[i];
            return sum * 0.5;
        }
    }
}
